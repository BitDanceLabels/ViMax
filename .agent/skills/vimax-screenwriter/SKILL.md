---
name: ViMax Screenwriter Agent
description: Transform raw ideas into complete stories and professional screenplays with scene divisions
---

# 🤖 ViMax Screenwriter Agent

Biến ý tưởng thô thành câu chuyện hoàn chỉnh và kịch bản chuyên nghiệp.

## Capabilities

1. **Develop Story** - Mở rộng ý tưởng thành câu chuyện hoàn chỉnh
2. **Write Script** - Chuyển câu chuyện thành kịch bản phân cảnh

---

## 🎬 Task 1: Develop Story from Idea
...
## 📝 Task 2: Write Script from Story
...
Output as JSON:
{
  "script": ["Scene 1 content...", "Scene 2 content...", ...]
}
...
- Visual Enhancement Principles: All descriptions must be "filmable". Use concrete actions instead of abstract emotions (e.g., "He turns away to avoid eye contact" instead of "He feels ashamed"). Describe rich environmental details include lighting, props, weather, etc., to enhance the atmosphere. Visualize character performances such as express internal states through facial expressions, gestures, and movements (e.g., "She bites her lip, her hands trembling" to imply nervousness).
...
```python
# Step 1: Develop story from idea
idea = "Nếu một con mèo và một con chó là bạn thân, điều gì sẽ xảy ra khi họ gặp một con mèo mới?"
user_requirement = "Dành cho trẻ em, không quá 3 cảnh, phong cách hoạt hình."

story = await screenwriter.develop_story(idea, user_requirement)

# Step 2: Convert story to script
script_scenes = await screenwriter.write_script_based_on_story(story, user_requirement)
```
